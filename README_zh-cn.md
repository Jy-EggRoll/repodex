...
RepoDex（RepositoryIndex——仓库索引聚合）：基于 GitHub Actions、Cloudflare Workers 和 React + Vite + Kumo 前端，实现了一套**跨仓库、跨分支的多维度模糊搜索系统**。

- 在线试用（演示数据为虚构样例，搜索逻辑与正式版一致）：[RepoDex Demo](https://jy-eggroll.github.io/repodex/)
- 博客文章链接：[项目介绍-RepoDex](https://eggroll.pages.dev/p/项目介绍-repodex/)
...
为此，我开发了本项目，实现了一套跨仓库、跨分支的多维度模糊搜索系统，以提升用户在 GitHub 上的文件检索效率。
...
- 界面中英双语（自动识别浏览器语言，可手动切换并记住选择）。所有可见字符串都经过 l10n 包装（VSCode l10n 风格：英文原文作 key，文案包位于 `/l10n`）
...
pnpm l10n:sort      # 按 key 排序 /l10n 文案包
...
- <https://www.i18next.com> i18next / <https://react.i18next.com> react-i18next，提供 i18n 运行时；文案包布局参考 VSCode l10n 约定。
